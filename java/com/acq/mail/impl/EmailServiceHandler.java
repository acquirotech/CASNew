...
public class EmailServiceHandler implements AcqEmailServiceHandler {
	...
	@Value("#{AcqConfig['merchantPayout_cc']}")
	 private String merchantPayoutCC ;
	 public String getMerchantPayoutCC() {
	  return merchantPayoutCC;
	 }
    ...
    private static void addAttachment(String filename,Multipart multipart, byte[] byteArray) throws MessagingException{
	    //DataSource source = new FileDataSource(filename);
	    DataSource source = new ByteArrayDataSource(byteArray, "application/vnd.ms-excel");
	    BodyPart messageBodyPart = new MimeBodyPart();        
	    messageBodyPart.setDataHandler(new DataHandler(source));
	    //messageBodyPart.setFileName(new File(filename).getName());
	    messageBodyPart.setFileName("payout_"+filename+".xls");
	    multipart.addBodyPart(messageBodyPart);
	}
    
    public void sendEmailHtmlBody(String date,String filename,final HashMap<String, Object> model,
			final String htmlBody, byte[] byteArray,final String toEmail) throws MailException{
    	try{
	  		...
	  		message.addRecipient(Message.RecipientType.CC,new InternetAddress(merchantPayoutCC));
	  		Multipart multipart = new MimeMultipart();
	  		BodyPart htmlPart = new MimeBodyPart();
	  		htmlPart.setContent(htmlBody, "text/html");
	  		htmlPart.setDisposition(BodyPart.INLINE);
			multipart.addBodyPart(htmlPart);
			message.setSubject("Acquiro Alert! Payout History "+date);
	  		addAttachment(filename,multipart, byteArray);
	  		message.setContent(multipart);
	  		Transport.send(message);
	  		System.out.println("Msg Send ....");
    	}catch(Exception e){
			logger.info("Error to payout email "+e);
    	}
	}
    ...
    public void sendEmailWithCC(HashMap<String, Object> model,
    	...
    	try {
			...
			try{
		         ...
		         if(model.get("mailType").equals("addMerchant")){
		        	 message.addRecipient(Message.RecipientType.CC,new InternetAddress("parul@acquiropayments.com"));
		         }
				 ...
				 String body = text.substring(text.indexOf(newline) + 1,text.length());
		...
		try {
			...
			try{
		        ...
		        // MimeMessageHelper mimeMessageHelper = new MimeMessageHelper(message, true);
		         ...
		         message.setContent(body,"text/html" ); 
		         MimeBodyPart attachPart = new MimeBodyPart();
		         String attachFile = "C:/Users/Parul Tripathi/Desktop/acq.jpg";		          
